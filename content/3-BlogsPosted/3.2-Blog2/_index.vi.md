...
Khi người dùng nhập thông tin và nhấn Đăng nhập trên trang `https://www.learnspherev2.id.vn`, trình duyệt sẽ chặn đứng yêu cầu gửi mật khẩu về cổng API `http://.../api/auth/login` để tránh bị đánh cắp dữ liệu, gây ra lỗi `Failed to fetch` trên màn hình.
...
```
1. **Yêu cầu Chứng chỉ**: Sử dụng AWS Certificate Manager (ACM) đăng ký chứng chỉ SSL miễn phí cho tên miền riêng: `learnspherev2.id.vn` và `*.learnspherev2.id.vn` (tại vùng Singapore).
...
3. **Trỏ DNS**: Tại trang Tenten.vn, tạo bản ghi CNAME trỏ tên miền phụ `api.learnspherev2.id.vn` về địa chỉ DNS của ALB.

Nhờ đó, API Backend đã có đường dẫn bảo mật chính chủ: `https://api.learnspherev2.id.vn/api/`!
...
1. Đăng ký chứng chỉ SSL ACM cho tên miền `learnspherev2.id.vn` tại vùng **N. Virginia (us-east-1)** (yêu cầu bắt buộc của CDN CloudFront).
2. Gắn tên miền phụ `www.learnspherev2.id.vn` và chứng chỉ SSL Mỹ vừa tạo vào CloudFront Distribution.
