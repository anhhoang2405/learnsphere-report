...
When a user entered their credentials on `https://www.learnsphere.id.vn` and clicked Login, the browser blocked the API call to `http://18.143.151.54:5000/api/auth/login` to prevent data sniffing, triggering a fatal `TypeError: Failed to fetch` console error.
...
```
1. **Request Certificate**: Issue free SSL certificates using AWS Certificate Manager (ACM) for our custom domain: `learnsphere.id.vn` and `*.learnsphere.id.vn`.
...
3. **Register DNS CNAME**: Point `api.learnsphere.id.vn` in our domain settings to the Load Balancer DNS.

Now, all API endpoints are securely accessible at `https://api.learnsphere.id.vn/api/`!

---

#### 4. Frontend SSL & CloudFront Configuration
For the frontend, we use Amazon CloudFront to serve the static build files from S3:
1. Issue another ACM SSL certificate for `www.learnsphere.id.vn` in the **US East (N. Virginia - `us-east-1`)** region (a global requirement for CloudFront).
...
#### 5. Conclusion
With both Frontend (`https://www.learnsphere.id.vn`) and Backend API (`https://api.learnsphere.id.vn`) fully encrypted on HTTPS, the Mixed Content security blocker is satisfied. Data transmitted between the user's browser and AWS is encrypted end-to-end, making our system secure and professional.
